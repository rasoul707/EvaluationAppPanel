...
const ResultComment = ({ match }) => {
    ...
    const load = () => {
        axios.get(`${BASE_URL}/api/comment/?commentEvaluate_id=${id}`, getDefaultHeaders())
            .then(res => {
                setLoading(false);
                setData(res.data)
                setSoftwareName(res?.data[0]?.commentEvaluate.software.software_name);
