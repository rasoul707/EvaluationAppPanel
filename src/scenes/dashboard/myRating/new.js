...
const NewMyRating = ({ match }) => {
    ...
    axios.get(`${BASE_URL}/api/rankEvaluate/?created_by=&software_id=${software}`, getDefaultHeaders()).then(res => {
        if (res.data.length > 0) {
            ...
            if (id) {
                ...
                // create
                axios.post(`${BASE_URL}/api/rankEvaluate/`, data, getDefaultHeaders())
                    .then(res => {
                        Swal.fire({
    ...
    const loadObject = () => {
        axios.get(`${BASE_URL}/api/rankEvaluate/${id}/`, getDefaultHeaders()).then(res => {
